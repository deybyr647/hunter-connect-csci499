...
const createPost = async (body: Partial<PostInterface>, bearerToken: string) => {
...
const updatePost = async (body: Partial<PostInterface>, bearerToken: string) => {
    ...
    try {
        ...
        const req = await fetch(`${API_URL}/api/posts/${postID}`, updatePostRequest);
        const json = await req.json();
