...
const updateEvent = async (body: EventApiRequest, bearerToken: string) => {
    ...
    try {
        const req = await fetch(`http://localhost:8080/api/events/${id}`, requestConfig);
        const json: EventInterface = await req.json();
...
const toggleSubscribe = async (body: EventApiRequest, bearerToken: string) => {
    ...
    try {
        const req = await fetch(`http://localhost:8080/api/events/${id}/subscribe`, requestConfig);
 ...
 */
export const timestampToDate = (ts: FirestoreTimestamp | null | undefined): Date | null => {
 ...
 */
export const formatDateString = (ts: FirestoreTimestamp | null | undefined): string => {
 ...
 */
export const formatTimeString = (ts: FirestoreTimestamp | null | undefined): string => {
    const date = timestampToDate(ts);
    if (!date) return "";
 ...
 */
export const formatTimeRange = (start: FirestoreTimestamp, end: FirestoreTimestamp): string => {
